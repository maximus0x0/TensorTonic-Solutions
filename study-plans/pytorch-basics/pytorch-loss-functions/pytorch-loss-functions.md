...
$$
L_i=\log\left(\sum_j e^{z_{i,j}-m_i}\right)-(z_{i,t_i}-m_i)
$$
...
<span style="font-size: 14px;">Use stable tensor operations; torch.logsumexp is allowed. Keep the correct-class logit shifted as well, so a large common offset does not cause cancellation. Applying softmax first and taking its logarithm later is mathematically related, but it recreates the numerical problem this exercise is intended to avoid.</span>
...
* <span style="font-size: 14px;">Predictions are supplied as a float32 tensor.</span>
* <span style="font-size: 14px;">For MSE, the supplied float32 targets support element-wise squared errors and mean reduction.</span>
* <span style="font-size: 14px;">For cross-entropy, use the supplied int64 class indices to select the correct shifted logit in each row.</span>
* <span style="font-size: 14px;">For Huber, the supplied float32 targets support both loss branches before mean reduction.</span>
